...
func TestClassify_Writes(t *testing.T) {
	...
	tests := []struct {
		...
		{"insert or replace", "INSERT OR REPLACE INTO users (id, name) VALUES (1, 'alice')", core.OpWrite, core.SubInsert},
		{"insert or ignore", "INSERT OR IGNORE INTO users (name) VALUES ('alice')", core.OpWrite, core.SubInsert},
...
func TestClassify_DDL(t *testing.T) {
	...
	tests := []struct {
		...
		{"create table", "CREATE TABLE users (id INTEGER PRIMARY KEY)", core.OpDDL, core.SubCreate},
		{"create if not exists", "CREATE TABLE IF NOT EXISTS users (id INTEGER)", core.OpDDL, core.SubCreate},
		{"create temp table", "CREATE TEMP TABLE scratch (id INTEGER)", core.OpDDL, core.SubCreate},
		{"alter table", "ALTER TABLE users ADD COLUMN age INTEGER", core.OpDDL, core.SubAlter},
		...
		{"create unique index", "CREATE UNIQUE INDEX idx_email ON users (email)", core.OpDDL, core.SubCreate},
		{"drop index", "DROP INDEX IF EXISTS idx_name", core.OpDDL, core.SubDrop},
	}
...
func TestClassify_Other(t *testing.T) {
	...
	tests := []struct {
		...
		{"explain query plan", "EXPLAIN QUERY PLAN SELECT * FROM users"},
		{"vacuum", "VACUUM"},
		{"reindex", "REINDEX"},
		{"analyze", "ANALYZE"},
...
func TestClassify_TableExtraction(t *testing.T) {
	...
	tests := []struct {
		...
		{"create", "CREATE TABLE items (id INTEGER PRIMARY KEY)", []string{"items"}},
		{"quoted table", `SELECT * FROM "users"`, []string{"users"}},
	}
