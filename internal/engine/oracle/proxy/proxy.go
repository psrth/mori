...
// Proxy is a protocol-aware Oracle proxy that classifies queries and routes
...
// New creates an Oracle Proxy.
func New(prodAddr, shadowAddr string, listenPort int, verbose bool,
...
func (p *Proxy) ListenAndServe(ctx context.Context) error {
	...
	if p.canRoute() {
		log.Printf("Mori Oracle proxy listening on %s → prod=%s shadow=%s", ln.Addr().String(), p.prodAddr, p.shadowAddr)
	} else {
		log.Printf("Mori Oracle proxy listening on %s → %s (pass-through)", ln.Addr().String(), p.prodAddr)
	}
...
func (p *Proxy) Shutdown(ctx context.Context) error {
	...
	select {
	case <-done:
		log.Println("All connections drained. Oracle proxy stopped.")
		p.logger.Close()
		return nil
	case <-ctx.Done():
		p.logger.Close()
