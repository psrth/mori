...
func TestClassify_ExecBlocked(t *testing.T) {
	c := New(nil)
	tests := []struct {
		name    string
		sql     string
		...
		{"exec sp_help", "EXEC sp_help", core.OpWrite, core.SubNotSupported},
		{"execute my_proc", "EXECUTE my_proc", core.OpWrite, core.SubNotSupported},
		{"exec with args", "EXEC my_proc @param1 = 1", core.OpWrite, core.SubNotSupported},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if cl.NotSupportedMsg == "" {
				t.Error("NotSupportedMsg should not be empty for EXEC/EXECUTE")
			}
		})
	}
}

func TestClassify_ExecSpRenameStillDDL(t *testing.T) {
	c := New(nil)
	cl, err := c.Classify("EXEC sp_rename 'users.old_col', 'new_col', 'COLUMN'")
	if err != nil {
