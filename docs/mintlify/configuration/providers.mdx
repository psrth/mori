...
| GCP Cloud SQL | PostgreSQL, MySQL, MSSQL | `require` | Password or `gcloud` IAM token |
| AWS RDS / Aurora | PostgreSQL, MySQL, MariaDB, MSSQL | `require` | Password or AWS IAM token |
| Neon | PostgreSQL | `require` | Password |
| Supabase | PostgreSQL | `require` | Password |
| Azure Database | PostgreSQL, MySQL, MariaDB, MSSQL | `require` | Password or Azure CLI token |
| PlanetScale | MySQL | `require` (TLS always) | Password |
| Vercel Postgres | PostgreSQL | `require` | Password |
| DigitalOcean | PostgreSQL, MySQL, Redis | `require` | Password |
| Railway | PostgreSQL, MySQL, Redis | `require` | Password |
...
<AccordionGroup>
  ...
  <Accordion title="GCP Cloud SQL">
    ...
    **With password:** Provide host, user, and password as normal. SSL enforced to `require`.

    **With IAM auth:** Omit the password. Mori obtains a token via `gcloud auth print-access-token`. Requires Google Cloud SDK installed and authenticated (`gcloud auth login`).
  ...
  <Accordion title="PlanetScale">
    **Provider ID:** `planetscale`
    ...
    Serverless MySQL. TLS always enabled — the provider hardcodes `tls=true` in the DSN.
