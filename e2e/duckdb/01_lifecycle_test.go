...
func TestLifecycle(t *testing.T) {
	...
	t.Run("proxy_accepts_current_timestamp", func(t *testing.T) {
		conn := connect(t)
		assertNoError(t, conn, "SELECT current_timestamp")
	})

	t.Run("status_shows_duckdb_engine", func(t *testing.T) {
		out := runMoriCommand(t, "status")
		if !strings.Contains(strings.ToLower(out), "duckdb") {
			t.Errorf("status output missing duckdb:\n%s", out)
		}
	})
	...
	t.Run("duckdb_version_query", func(t *testing.T) {
		conn := connect(t)
		ver := queryString(t, conn, "SELECT version()")
		if ver == "" {
