...
var (
	proxyDSN   string
	prodDBPath string // Absolute path to the prod DuckDB file.
	proxyPort  = 19008
...
func runTests(m *testing.M) int {
	// Determine the repo root (parent of e2e/duckdb/).
	...
	var err error
	projectDir, err = os.MkdirTemp("", "mori-e2e-duckdb-*")
	if err != nil {
	...
	// 3. Create prod DuckDB database file.
	fmt.Println("SETUP: Creating prod DuckDB database...")
	prodDBPath = filepath.Join(projectDir, "prod.duckdb")
...
// createAndSeedProdDB creates the prod DuckDB file, applies the DDL schema,
// and inserts seed data using Go loops.
func createAndSeedProdDB(ctx context.Context) error {
	db, err := sql.Open("duckdb", prodDBPath)
	if err != nil {
	...
	seedPath := filepath.Join(repoRoot, "e2e", "duckdb", "seed.sql")
	...
	for _, stmt := range stmts {
		...
		if _, err := db.ExecContext(ctx, stmt); err != nil {
			return fmt.Errorf("executing seed SQL %q: %w", truncSQL(stmt), err)
		}
	}
...
// splitStatements splits SQL text on semicolons (naive but sufficient for seed.sql).
...
func seedData(db *sql.DB) error {
	...
	// 100 users (sequences handle auto-increment).
	for i := 1; i <= 100; i++ {
		isActive := 1
		if i%10 == 0 {
			isActive = 0
	...
	for _, name := range roleNames {
		_, err := tx.Exec("INSERT INTO roles (name, description) VALUES ($1, $2)",
	...
	for i := 1; i <= 150; i++ {
		_, err := tx.Exec(
			"INSERT INTO user_roles (user_id, role_id, granted_at) VALUES ($1, $2, current_timestamp) ON CONFLICT DO NOTHING",
			((i-1)%100)+1, ((i-1)%10)+1,
		)
		if err != nil {
			tx.Rollback()
	...
	for i := 1; i <= 50; i++ {
		_, err := tx.Exec(
			"INSERT INTO products (id, name, slug, price) VALUES ($1, $2, $3, $4)",
	...
	for _, s := range settingsList {
		_, err := tx.Exec("INSERT INTO settings (key, value) VALUES ($1, $2)", s[0], s[1])
		if err != nil {
			tx.Rollback()
...
// writeMoriYAML writes a mori.yaml file for the DuckDB connection.
