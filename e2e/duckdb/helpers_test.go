...
// connectDirect returns a *sql.DB connected directly to the prod DuckDB file.
func connectDirect(t *testing.T) *sql.DB {
	t.Helper()
	db, err := sql.Open("duckdb", prodDBPath+"?access_mode=READ_ONLY")
	if err != nil {
...
// queryScalar runs a query through proxy returning a single value.
...
// mustExecDirect runs Exec directly on prod DuckDB file.
...
// mustQueryDirect runs a query directly on prod DuckDB and returns rows as maps.
