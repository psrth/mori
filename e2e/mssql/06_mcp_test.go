...
func TestMCP(t *testing.T) {
	t.Run("select_query_returns_rows", func(t *testing.T) {
		result, err := queryMCP(t, "SELECT TOP 3 id, username FROM users")
		if err != nil {
	...
	t.Run("insert_via_mcp", func(t *testing.T) {
		result, err := queryMCP(t, "INSERT INTO settings ([key], value) VALUES ('mcp-tag', 'mcp-value')")
		if err != nil {
	...
	t.Run("insert_then_select_via_mcp", func(t *testing.T) {
		...
		result, err := queryMCP(t, "SELECT value FROM settings WHERE [key] = 'mcp-verify'")
		if err != nil {
			t.Fatalf("MCP SELECT: %v", err)
		}
		if !strings.Contains(result, "mcp-verify-val") {
			t.Errorf("expected mcp-verify-val in result, got: %s", truncSQL(result))
		}
	})
	...
	t.Run("ddl_via_mcp", func(t *testing.T) {
		result, err := queryMCP(t, "CREATE TABLE mcp_test_table (id INT IDENTITY(1,1) PRIMARY KEY, name NVARCHAR(255))")
		if err != nil {
