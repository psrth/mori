...
func TestLifecycle(t *testing.T) {
	...
	t.Run("proxy_accepts_datetime_function", func(t *testing.T) {
		db := connect(t)
		assertNoError(t, db, "SELECT GETDATE()")
	})

	t.Run("status_shows_mssql_engine", func(t *testing.T) {
		out := runMoriCommand(t, "status")
		if !strings.Contains(out, "Engine:") {
			t.Errorf("status output missing Engine info:\n%s", out)
		}
		if !strings.Contains(strings.ToLower(out), "mssql") {
			t.Errorf("status output missing mssql:\n%s", out)
		}
	})
	...
	t.Run("db_name_via_proxy", func(t *testing.T) {
		db := connect(t)
		name := queryScalar[string](t, db, "SELECT DB_NAME()")
		if name != dbName {
			t.Errorf("DB_NAME() = %q, want %q", name, dbName)
		}
	})
