...
func connectDirect(t *testing.T) *sql.DB {
	t.Helper()
	dsn := fmt.Sprintf("%s:%s@tcp(127.0.0.1:%d)/%s", dbUser, dbPass, prodPort, dbName)
...
// mustExecResult runs Exec and returns the sql.Result.
func mustExecResult(t *testing.T, db *sql.DB, query string, args ...any) sql.Result {
	...
	if err != nil {
		t.Fatalf("mustExecResult(%q): %v", truncSQL(query), err)
	}
	return res
}
