...
func TestJoins(t *testing.T) {
	...
	t.Run("left_join_users_orders", func(t *testing.T) {
		rows := mustQuery(t, db,
			`SELECT u.id, u.username, o.status
...
func TestSubqueries(t *testing.T) {
	...
	t.Run("subquery_in_where_in", func(t *testing.T) {
		rows := mustQuery(t, db,
			`SELECT username FROM users
			 WHERE id IN (SELECT user_id FROM orders WHERE status = 'completed' FETCH FIRST 5 ROWS ONLY)`)
		if len(rows) < 1 {
	...
	t.Run("subquery_scalar", func(t *testing.T) {
		rows := mustQuery(t, db,
			`SELECT username, (SELECT COUNT(*) FROM orders o WHERE o.user_id = u.id) AS order_count
...
func TestCTEs(t *testing.T) {
	...
	t.Run("simple_cte", func(t *testing.T) {
		rows := mustQuery(t, db,
			`WITH active_users AS (
				SELECT id, username FROM users WHERE is_active = 1 FETCH FIRST 20 ROWS ONLY
			)
	...
	t.Run("multiple_ctes", func(t *testing.T) {
		rows := mustQuery(t, db,
			...
			top_users AS (
				SELECT user_id, cnt FROM user_orders ORDER BY cnt DESC FETCH FIRST 10 ROWS ONLY
			)
	...
	t.Run("having_clause", func(t *testing.T) {
		rows := mustQuery(t, db,
			`SELECT user_id, COUNT(*) AS cnt FROM orders
			 ...
			 FETCH FIRST 10 ROWS ONLY`)
		if len(rows) < 1 {
			t.Errorf("expected rows from HAVING query, got %d", len(rows))
		}
	})
}
