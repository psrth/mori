...
func TestLifecycle(t *testing.T) {
	...
	t.Run("status_shows_mariadb_engine", func(t *testing.T) {
		...
		if !strings.Contains(strings.ToLower(out), "mariadb") {
			t.Errorf("status output missing mariadb:\n%s", out)
		}
	})
	...
	t.Run("version_query", func(t *testing.T) {
		...
		if !strings.Contains(strings.ToLower(ver), "mariadb") {
			t.Logf("VERSION() does not contain 'mariadb': %s", ver)
		}
		t.Logf("MariaDB version: %.80s", ver)
	})
}
