...
func TestLifecycle(t *testing.T) {
	t.Run("proxy_accepts_simple_query", func(t *testing.T) {
		conn := connect(t)
		val := queryScalar[int32](t, conn, "SELECT 1")
		if val != 1 {
	...
	t.Run("status_shows_table_info", func(t *testing.T) {
		out := runMoriCommand(t, "status")
		if !strings.Contains(out, "Sequence Offsets:") {
			t.Errorf("status output missing Sequence Offsets:\n%s", out)
		}
	})
	...
	t.Run("multiple_connections_work", func(t *testing.T) {
		conn1 := connect(t)
		conn2 := connect(t)
		v1 := queryScalar[int32](t, conn1, "SELECT 1")
		v2 := queryScalar[int32](t, conn2, "SELECT 2")
		if v1 != 1 || v2 != 2 {
