...
func TestBasicSelect(t *testing.T) {
	...
	t.Run("select_with_where_uuid_pk", func(t *testing.T) {
		// Products use deterministic UUIDs from the seed pattern.
...
func TestBasicInsert(t *testing.T) {
	...
	t.Run("insert_serial_pk_returns_id", func(t *testing.T) {
		var id int64
	...
	t.Run("insert_with_text_ip_value", func(t *testing.T) {
		// CockroachDB uses TEXT instead of INET for IP addresses.
		mustExec(t, conn,
			`INSERT INTO users (username, email, password_hash, last_login_ip) VALUES ('e2e_ip_user', 'ip@test.com', 'hash', '10.20.30.40')`)
	})
