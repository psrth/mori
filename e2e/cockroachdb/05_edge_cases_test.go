	...
	t.Run("parallel_inserts", func(t *testing.T) {
		...
		for i := 0; i < 5; i++ {
			...
			go func(idx int) {
				...
				defer c.Close(ctx)
				// Use literal SQL instead of parameterized to avoid binary format
				// mismatch between CockroachDB INT8 and PostgreSQL shadow INT4.
				_, errors[idx] = c.Exec(ctx,
					fmt.Sprintf("INSERT INTO audit_log (user_id, action, resource, resource_id) VALUES (%d, 'parallel_test', 'test', 'parallel')", idx+1))
			}(i)
		}
